...
def checkleapyear(year1):
    if year1 % 4 == 0:
        ...
        return 365
# End Code Checking of Leap Year

# Finding Date diff between Two dates(array)=[Year, Month ,Day, Hour, Min]
def daydiff(dat1, dat2):
    if dat1[0] < dat2[0] and dat1[1] in range(1, 13) and dat2[1] in range(1, 13):
        nday = 0
        for i in range(int(dat1[0] + 1), int(dat2[0])):
            nday += checkleapyear(i)
        monthlist = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
        if checkleapyear(dat1[0]) == 366:
            monthlist[1] = 29
        else:
            monthlist[1] = 28
        for i in range(dat1[1], 12):
            nday += monthlist[i]
        nday = nday + monthlist[dat1[1] - 1] - dat1[2] + 1
        if checkleapyear(dat2[0]) == 366:
            monthlist[1] = 29
        else:
            monthlist[1] = 28
        for i in range(0, int(dat2[1]) - 1):
            nday += monthlist[i]
        nday += dat2[2]
        nday += -((dat1[3] + dat1[4] / 60) / 24) + ((dat2[3] + dat2[4] / 60) / 24) - 1
        return nday
    else:
        return np.nan
# End Code Finding Date diff between Two dates
...
def main():
    ...
    for i in range(1, np.shape(Year)[0]):
        ...
        Daydiff[i] = float(daydiff(Ref_Date, To_Date))
